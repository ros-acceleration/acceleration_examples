...
that leverages type adaptation and targeting the CPUs for the computations.
*/
...
#include "doublevadd_component_typeadapter.hpp"
#include "vadd.hpp"
...
// Create a DoubleVaddTypeAdapter "component" that subclasses the generic rclcpp::Node base class.
// Components get built into shared libraries and as such do not write their own main functions.
// The process using the component's shared library will instantiate the class as a ROS node.
DoubleVaddTypeAdapter::DoubleVaddTypeAdapter(const rclcpp::NodeOptions & options)
: Node("doublevadd_publisher", options), count_(0)
{
  // Create a publisher of "std_mgs/String" messages on the "chatter" topic.
  // pub_ = create_publisher<MyAdaptedType>("vector", 10);
  pub_ = this->create_publisher<MyAdaptedType>("topic", 10);

  // define differential time value (in ms) for the loop
  dt_ = 100ms;  // 10 Hz

  // Use a timer to schedule periodic message publishing.
  timer_ = create_wall_timer(dt_, std::bind(&DoubleVaddTypeAdapter::on_timer, this));

}

void DoubleVaddTypeAdapter::on_timer()
{
  ...
  std::string message = "vadd finished, iteration: " +
...
RCLCPP_COMPONENTS_REGISTER_NODE(composition::DoubleVaddTypeAdapter)

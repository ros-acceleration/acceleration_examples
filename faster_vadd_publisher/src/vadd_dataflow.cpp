...
Inspired by the hello_world Vitis example.
See https://raw.githubusercontent.com/Xilinx/Vitis_Accel_Examples/master/hello_world/src/vadd.cpp
...
static void compute_add(hls::stream<hls::vector<unsigned int, 16> >& in1_stream,
                        ...
                        int vSize) {
// The kernel is operating with SIMD vectors of 16 integers.
// The + operator performs an element-wise add, resulting in
// 16 parallel additions.
execute2:
...
extern "C" {
    ...
    */
    ...
    #pragma HLS INTERFACE m_axi port = out bundle = gmem0
        ...
        static hls::stream<hls::vector<unsigned int, 16> >
            in1_stream("input_stream_1");
        static hls::stream<hls::vector<unsigned int, 16> >
            in2_stream("input_stream_2");
        static hls::stream<hls::vector<unsigned int, 16> >
            out_stream("output_stream");

        // Since 16 values are processed
        // in parallel per loop iteration, the for loop only
        // needs to iterate 'size / 16' times.
        assert(size % 16 == 0);
        int vSize = size / 16;
    #pragma HLS dataflow
